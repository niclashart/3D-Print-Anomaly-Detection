...
dict = {"Anomalie": 0, "Standard-Schraube": 1}  # class names for cnn
...
# Training
model, history = create_cnn_model(
    epoch, batch_size, train_images, train_labels, val_images, val_labels
)
...
for image, label in zip(train_images, train_labels):
    if isinstance(image, str):
        ...
        print(
            f"Loaded image: {image.shape if image is not None else None}"
